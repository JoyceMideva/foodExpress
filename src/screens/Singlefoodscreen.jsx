...
const Singlefoodscreen = ({route,navigation}) => {
  ...
  return (
    <View>
      <View className="flex items-center  bg-cover bg-center bg-no-repeat bg-[linear-gradient(rgba(0,0,0,.5),rgba(0,0,0,.5)),url('assets/mokimo.jpg')]"></View>
      <Image
      ...
      <View className="flex flex-row items-center justify-between gap-8 shadow-2xl">
        ...
        <View className="flex flex-row items-center justify-between gap-8">
          <Text className="bg-gray-400 rounded-full px-[2em] py-2">-</Text>
      ...
      onPress={()=>{addToCart(singlefood)}}
        className="font-bold text-2xl text-start text-black"
        title="Add To Order"
      />
      <TouchableOpacity onPress={()=>navigation.navigate("Cart")}>
       <Text> navigateToCart</Text>
      </TouchableOpacity>
    </View>
  );
};
